...
static void MyDirectoryListingCallBack(CFReadStreamRef readStream, CFStreamEventType type, void * clientCallBackInfo)
{
    ...
    switch (type) {
            ...
            if (bytesRead > 0) {
                ...
                do
                {
                    ...
                    if (bytesConsumedThisTime > 0) {
                        if (parsedDict != NULL) {
                            [[FTPHelper sharedInstance].fileListings addObject:(__bridge NSDictionary *)parsedDict];
        ...
        case kCFStreamEventEndEncountered:
            ...
            goto exit;
        default:
            fprintf(stderr, "Received unexpected CFStream event (%lu)", type);
            break;
    }
    return;
...
- (void) list
{
    ...
    MySimpleDirectoryListing((__bridge CFStringRef)self.urlString, (__bridge CFStringRef) uname, (__bridge CFStringRef) pword);
}
...
static Boolean MySimpleUpload(CFStringRef uploadDirectory, CFURLRef fileURL, CFStringRef username, CFStringRef password)
{
    ...
    if (success) {
        ...
        if (success) {
            ...
            //CFWriteStreamSetProperty(writeStream, kCFStreamPropertySSLPeerTrust, kCFBooleanFalse);
...
//- (void) put: (NSString *) fileLocation totalUploadBytes:(SInt64)totalBytes
- (void) put: (NSString *) fileLocation
{
//    totalBytesUpload = totalBytes;
//    printf("totalBytesUpload ------->>>>> %lld\n", totalBytesUpload);
    
    if (!self.uname || !self.pword) COMPLAIN_AND_BAIL(@"Please set user name and password first");
    if (!self.urlString) COMPLAIN_AND_BAIL(@"Please set URL string first");
    NSLog(@"urlString:%@", self.urlString);
        NSString *readPath = fileLocation;
        CFURLRef readURL = CFURLCreateFromFileSystemRepresentation (NULL, (const UInt8 *) [readPath UTF8String], [readPath length], NO);
    MySimpleUpload((__bridge CFStringRef)self.urlString, readURL, (__bridge CFStringRef)uname, (__bridge CFStringRef)pword);
        CFRelease(readURL);
}
...
- (void) createDirectory : (NSString *)directoryPath
{
   ...
   // CFURLRef readURL = CFURLCreateFromFileSystemRepresentation (NULL, (const UInt8 *) [directoryPath UTF8String], [directoryPath length], NO);
    
    
    CFURLRef readURL = CFURLCreateCopyAppendingPathComponent(NULL, (__bridge CFURLRef)self.urlString, (__bridge CFStringRef) directoryPath, true);
...
+ (void) createDirectory : (NSString *)anItem
{
    [sharedInstance createDirectory:anItem];
}
//- (void) fetch: (NSString *) anItem destinationPath:(NSString *)destinationPath


//+ (void) upload: (NSString *) anItem uploadBytes:(SInt64 )totalBytes
+ (void) upload: (NSString *) anItem
{
    [sharedInstance put: anItem];
    
//    [sharedInstance put:anItem totalUploadBytes:totalBytes];
...
static Boolean MySimpleDownload(CFStringRef urlString, CFURLRef destinationFolder, CFStringRef username, CFStringRef password)
{
    ...
    [[NSFileManager defaultManager] fileExistsAtPath:[(__bridge NSURL *)destinationFolder path] isDirectory: &dirPath];
    if (!dirPath) {
    ...
    if (success) {
        ...
        if (success) {
            ...
            CFReadStreamSetProperty(readStream, kCFStreamPropertyFTPUsePassiveMode, kCFBooleanFalse);
...
- (void) fetch: (NSString *) anItem destinationPath:(NSString *)destinationPath
{
    ...
    CFShow((__bridge CFTypeRef)(ftpRequest));
    NSString *writepath = destinationPath;
   // self.filePath = [writepath stringByAppendingPathComponent:anItem];
    ...
    MySimpleDownload((__bridge CFStringRef)ftpRequest, writeURL, (__bridge CFStringRef) self.uname, (__bridge CFStringRef)self.pword);
}


+ (void) download:(NSString *) anItem destinationPath:(NSString *)destinationPath
{
    [sharedInstance fetch:anItem destinationPath:destinationPath];
}
#pragma mark Class
